...
const QuizEnd = ({ results, data, onReset, onAnswersCheck, time }) => {
  ...
  if(valueScore >= 85){
    ...
    window.location.href = 'http://ompfe.okular.co.id/id/quiz-category-01';
  } else if(valueScore >= 75 && valueScore < 85){
    console.log("Hasil Kategori 2 = "+valueScore);
    window.location.href = 'http://ompfe.okular.co.id/id/quiz-category-02';
  } else {
    console.log("Failed Score = "+valueScore);
    window.location.href = 'http://ompfe.okular.co.id/id/quiz-category-03';
  }
