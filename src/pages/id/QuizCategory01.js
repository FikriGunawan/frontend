...
class QuizCategory01 extends React.Component{
  ...
  componentDidMount(){
    fetch(`http://ompcms.okular.co.id/quiz-category-1?_locale=id`).then((response) => {
      if(response.status >= 400){
        throw new Error("Bad Response From Server");
      }
      return response.json();
    }).then((quizcat) => {
      this.setState({QuizCategory01Page: quizcat});
  ...
  render(){
    const {
      ...
      Category_Item2_Link
    } = this.state.QuizCategory01Page;
    return (
      <>
        {/* Navbar */}
        <Navbar
          langID='/id/quiz-category-01'
          langEN='/en/quiz-category-01'
        />
